...
| **C5** | No rate limits / budget on data plane | 8 | outsider | Flood `/v1/chat` | `DataPlaneLimiter` RPM + daily token/cost caps (env: `GLC_DATA_PLANE_RPM`, `GLC_DATA_PLANE_MAX_TOKENS_DAY`, `GLC_DATA_PLANE_MAX_COST_USD_DAY`) | Set `GLC_DATA_PLANE_RPM=2`, burst 3 POSTs → third **429**. |
