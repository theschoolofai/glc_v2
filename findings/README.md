...
a `tempfile.mkdtemp()` directory) rather than requiring a separately-running server — this is the
same mechanism the test suite's `tests/conftest.py::_isolated_glc_state` fixture uses, so
`uv run python findings/<id>/repro.py` is self-contained from a fresh checkout with no manual setup
beyond `uv sync`.
