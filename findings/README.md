...
`tests/conftest.py::_isolated_glc_state` fixture uses, so `uv run python findings/<id>/repro.py` is
self-contained from a fresh checkout with no manual setup beyond `uv sync`.
