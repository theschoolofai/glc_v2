...
from glc.config import (
    ...
    get_or_create_ledger_hmac_key,
)
from glc.security.idempotency import get_idempotency_store
from glc.security.isolation import ledger_signing_key
...
def test_p2_ledger_hmac_not_derived_from_install_token():
    """P2-A — ledger signing key must be independent of the install bearer."""
    install = get_or_create_install_token()
    ledger = get_or_create_ledger_hmac_key()
    assert install != ledger
    assert ledger_signing_key() == ledger.encode("utf-8")
    assert ledger_signing_key() != install.encode("utf-8")
...
async def test_p2_whatsapp_rejects_replayed_message_id(monkeypatch):
    ...
    # Exact same signed body again (classic webhook replay).
...
async def test_p2_webhook_rejects_replayed_body(monkeypatch):
    ...
    assert first is not None
    second = await adapter.on_message(ev)  # identical raw_body + still-fresh signature
    assert second is None


@pytest.mark.asyncio
async def test_p2_slack_public_requires_mention_for_paired_user(tmp_path, monkeypatch):
    """P2-C / invariant 2 — Slack must call allowlists.allowed() like peer channels."""
    # Packaged channels.yaml disables slack; enable it for this allowlist check.
    ...
    mock = SlackMock()
    store = get_pairing_store()
    code, _ = store.issue_code("slack", SLACK_STRANGER, "paired", requested_trust_level="user_paired")
