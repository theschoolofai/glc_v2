"""Mock-API fake for the whisper.cpp (local subprocess) STT provider.

Wire-format source: https://github.com/ggerganov/whisper.cpp
...
class WhisperCppMock:
    ...
    async def transcribe(self, audio: bytes, mime: str) -> TranscribeResult:
        ...
        if self.rate_limited:
            raise STTError("upstream rate-limited", status=421)
