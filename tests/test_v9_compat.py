...
def test_v9_routes_are_registered(auth_client):
...
def test_new_s11_routes_are_registered(auth_client):
    openapi = auth_client.get("/openapi.json").json()
    paths = set(openapi["paths"].keys())
    for p in [
        "/v1/transcribe",
...
def test_v1_providers_shape_unchanged(auth_client):
    body = auth_client.get("/v1/providers").json()
    # V9 shape: order, providers, shortcuts, limits, models
    for k in ("order", "providers", "shortcuts", "limits", "models"):
        assert k in body


def test_v1_status_shape_unchanged(auth_client):
    body = auth_client.get("/v1/status").json()
    for k in ("order", "live", "today", "limits"):
        assert k in body


def test_v1_capabilities_returns_per_provider_caps(auth_client):
    body = auth_client.get("/v1/capabilities").json()
    # Even with zero providers wired, the shape must be a dict.
    assert isinstance(body, dict)


def test_v1_cost_by_agent_returns_dict(auth_client):
    body = auth_client.get("/v1/cost/by_agent").json()
    assert isinstance(body, dict)


def test_chat_request_rejects_bad_provider(auth_client):
    r = auth_client.post("/v1/chat", json={"prompt": "hi", "provider": "no_such_provider"})
    # If no providers wired at all, the validation hits 400; if they are
    # wired, the candidate list is empty (also 400).
    assert r.status_code in (400, 503)


def test_chat_request_minimal_body_validates(auth_client):
...
def test_embed_request_413_on_oversize(auth_client):
    huge = "x" * 9000
    r = auth_client.post("/v1/embed", json={"text": huge})
...
def test_data_plane_requires_auth(app_client):
